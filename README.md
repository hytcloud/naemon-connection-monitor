...
PowerShell scripts for monitoring inbound connections to specific IP:Port combinations or process commands. Designed to run directly on the target host. Includes tools for checking current connection count and detecting reconnection events over time. Compatible with Windows PowerShell and PowerShell Core on Linux.
...
**Description**
Checks how many active inbound connections exist to the specified IP and port(s), or to a process command (Windows only). Supports multiple ports, threshold comparison using -count with a comparison -mode, and outputs standard Naemon plugin exit codes.
